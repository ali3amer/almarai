...
class Sale extends Model
{
    ...
    public function getMovements($id = null, $clientType = 'client')
    {
     ...
     END as clientType"),
            ...
            DB::raw('COALESCE(sales.client_id, sales.supplier_id, sales.employee_id) as owner_id'),
            DB::raw("COALESCE(clients.clientName, suppliers.supplierName, employees.employeeName) as owner_name"),  // إضافة اسم المالك
        ...
        $paidReturnSale = SaleReturn::select(
            ...
            DB::raw('null as type'),
            'sale_id as invoice_id',
            DB::raw('sale_returns.amount as paid'),
            DB::raw('0 as debt'),
            DB::raw('0 as futurePaid'),
            DB::raw("0 as futureDebt"),
            'sale_returns.due_date',
            ...
            DB::raw('COALESCE(sales.client_id, sales.supplier_id, sales.employee_id) as owner_id'),
            DB::raw("COALESCE(clients.clientName, suppliers.supplierName, employees.employeeName) as ownerName"),  // إضافة اسم المالك
            'sale_returns.created_at',
            'sale_returns.updated_at'
        )->where("sale_returns.amount", "!=", 0)->join('sales', 'sales.id', '=', 'sale_returns.sale_id')
            ->leftJoin('clients', 'clients.id', '=', 'sales.client_id')
            ->leftJoin('suppliers', 'suppliers.id', '=', 'sales.supplier_id')
            ->leftJoin('employees', 'employees.id', '=', 'sales.employee_id');
