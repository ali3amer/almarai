...
class Supplier extends Model
{
    ...
    public function getCurrentPurchasesBalanceAttribute()
    {
        ...
        return $this->initialBalance + $this->purchases()->sum("remainder") - $this->purchaseDebts()->where("type", "discount")->sum("amount") + $this->purchaseDebts()->where("type", "debt")->sum("amount") - $this->purchaseDebts()->where("type", "pay")->sum("amount") - $creditReturnsTotal;
    }
    public function getCurrentSalesBalanceAttribute()
    {
        ...
        return $this->initialSalesBalance + $this->sales()->sum("remainder") + $this->saleDebts()->where("type", "debt")->sum("amount") - $this->saleDebts()->where("type", "discount")->sum("amount") - $this->saleDebts()->where("type", "pay")->sum("amount") - $creditReturnsTotal;
    }
    ...
    public function getSalesPastBalance($date)
    {
        ...
        return $this->initialBalance + $this->sales()->where("due_date", "<", $date)->sum("remainder") - $this->saleDebts()->where("due_date", "<", $date)->where("type", "discount")->sum("amount") + $this->saleDebts()->where("type", "debt")->where("due_date", "<", $date)->sum("amount") - $this->saleDebts()->where("due_date", "<", $date)->where("type", "pay")->sum("amount") - $creditReturnsTotal;
    }
