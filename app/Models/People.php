...
class People extends Model
{
    ...
    public function getCurrentSalesBalanceAttribute()
    {
        ...
        return $this->initialSalesBalance + $this->sales()->sum("remainder") - $this->saleDebts()->where("type", "discount")->sum("amount") + $this->saleDebts()->where("type", "debt")->sum("amount") + $this->saleDebts()->where("type", "debt")->sum("amount") - $this->saleDebts()->where("type", "pay")->sum("amount") - $creditReturnsTotal;
    }
