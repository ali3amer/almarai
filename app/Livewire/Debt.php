    ...
    public function payDebt()
    {
        if ($this->debtId == 0) {
            ...
            if ($this->buyer == 'client') {
                ...
                \App\Models\Supplier::where('id', $this->currentClient['id'])->increment('currentBalance', $this->debtPaid);
            }
        ...
        $this->reset('debtId', 'debtPaid', 'remainder', 'currentDebt', 'bank', 'payment', 'due_date');
    }

    public function chooseDebt($id)
    {
        $this->debtId = $id;
        $debt = SaleDebt::where('id', $id)->first()->toArray();
