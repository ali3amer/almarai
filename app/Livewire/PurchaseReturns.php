...
//            $payment = $purchase->purchaseDebts->where("type", "pay")->first()->paid;

            if ($purchase->paid > 0 && floatval($this->priceReturn) <= floatval(session("safeBalance"))) {
                $this->confirm("المبلغ المرتجع أكبر من المبلغ المتوفر", [
                    'toast' => false,
